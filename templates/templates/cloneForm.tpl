...
<form id="cloneForm" method="post" action="clone.php?event={$id}"
	...
	<table>
		<tr>
			...
			<td class="clone"><input {if $warnName} class="warn"{/if}name="name" id="name" size="20" tabindex="1" value="{$name}" maxlength="{$smarty.const.MAX_EVENT_TITLE_LENGTH}" /></td>
		</tr><tr>
...
{/section}
			...
			<td class="clone">
				...
				<input name="owner" value="{$owner}"{if $cloneOwner && !$nojs} disabled="disabled"{/if} maxlength="{$smarty.const.MAX_MAIL_ADDRESS_LENGTH}" />
			</td>
		</tr><tr>
