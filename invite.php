 ...
 *
 * $Id: invite.php 100 2006-04-26 00:47:10Z stefan $
 */
	...
	$eventID = $_GET['event'];
	if (empty($_POST['name'])) {
		header('location: show.php?id=' . $eventID . '&invite&warn=name');
		die();
	}
	if (empty($_POST['mail'])) {
		header('location: show.php?id=' . $eventID . '&invite&warn=mail');
		die();
	}

	$person = new Person($eventID);
	$person->setName($_POST['name']);
	$person->invite($eventID, $_POST['mail']);
	echo $person->save();
