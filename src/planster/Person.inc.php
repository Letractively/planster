 ...
 *
 * $Id: Person.inc.php 143 2006-05-01 01:33:31Z stefan $
 */
...
class Person extends DBItem {
	...
	function load() {
		$rs = $this->_conn->Execute('SELECT * FROM ' . dbTablePrefix . 'people WHERE person_id=?', $this->_id);
	...
	function setName($name) {
		$this->_name = str_replace('|', '&#124', $name);
	}
	...
	function getStatus($event_id, $date) {
		...
		$recordSet = $this->_conn->Execute($query, array($this->_id, $event_id, $date_id));

		if (count($recordSet->fields) <= 1) {
			return STATUS_UNKNOWN;
		} else {
			return $recordSet->fields['status'];
		}
	}
	...
	function setStatus($event_id, $status_array) {
		$this->_conn->Execute('DELETE FROM ' . dbTablePrefix . 'status WHERE person_id=? AND event_id=?', array($this->_id, $event_id));
		foreach ($status_array as $date_id => $status) {
			$query = 'INSERT INTO ' . dbTablePrefix . 'status SET person_id=?, event_id=?, date_id=?, status=?';
			$this->_conn->Execute($query, array($this->_id, $event_id, $date_id, $status));
		}
		$change = new Change(CHANGE_TYPE_MODIFY_ENTRY, $event_id, $this->_name);
		$change->save();
	}
	...
	function invite($eventID, $mail) {
		...
		$change = new Change(CHANGE_TYPE_INVITE, $eventID, $this->_name);
		$change->save();
	}
}
?>
